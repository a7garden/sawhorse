...
export function programSummary(rows: RequirementStatus[]): string {
  ...
  if (missing.length === 0 && outdated.length === 0) return `${rows.length}개 모두 준비됐습니다.`;
  const parts: string[] = [];
    ...
    <div className="space-y-3">
      <div className="flex items-start justify-between gap-2">
        <p className="text-xs leading-relaxed text-muted-foreground">
          이 PC에서 찾은 것만 표시합니다. 없는 항목의 <b className="text-foreground">설치</b> 를
          누르면 받는 곳이 브라우저로 열립니다. 설치한 뒤 <b className="text-foreground">다시 검사</b>.
        </p>
      ...
      <p className="text-[11px] text-muted-foreground">{programSummary(rows)}</p>
        ...
        <DetectRow
          ...
          badge={
            <>
              ...
              {r.outdated && <Badge variant="warning">{r.minMajor} 이상 필요</Badge>}
            </>
          }
          version={r.version}
          detail={r.why}
          ...
          action={
            r.detected && !r.outdated ? undefined : (
              <InstallButton url={r.installUrl} label={r.outdated ? "받기" : "설치"} />
            )
          }
        />
      ))}
      ...
      {blocking.length > 0 && (
        <p className="text-[11px] text-warning-foreground">
          필수 항목이 빠져 있습니다 ({blocking.map((r) => r.name).join(", ")}). 지금은 그냥
          넘어가도 되고, 설치한 뒤 설정 → 진단에서 다시 검사하면 됩니다.
        </p>
      )}
    </div>
  );
