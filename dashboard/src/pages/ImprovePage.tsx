...
import type { ImprovementNote } from "@/lib/types";
...
export default function ImprovePage() {
  ...
  const canImplement =
    selNotes.length > 0 &&
    selNotes.every((n) => n.approve || n.status === "구현중" || n.status === "부분구현");

  const detail = improvements.find((n) => n.path === detailPath) ?? null;
  const changeTargets = detailMd ? extractSection(detailMd, "변경 대상") : "";
  ...
  async function openDetail(n: ImprovementNote) {
  ...
  async function approve() {
    ...
    setBusy("approve");
    try {
      await api.approveNote(detailPath);
  ...
  async function run(kind: RunKind) {
    ...
    try {
      await api.enqueueJob({
        kind,
        project: kind === "excel" ? undefined : effectiveProject || undefined,
  ...
  return (
    <div>
      <PageHeader title="개선" desc="문제 노트를 검토하고 설계·구현을 큐에 등록합니다.">
        ...
        <Button
          size="sm"
          disabled={selNotes.length === 0 || busy !== null}
          onClick={() => void run("design")}
          title="선택한 문제의 설계를 작성합니다."
        >
          <PencilRuler /> 설계 실행{selNotes.length > 0 ? ` (${selNotes.length})` : ""}
        </Button>
        <Button
          size="sm"
          disabled={selNotes.length === 0 || !canImplement || busy !== null}
          onClick={() => void run("implement")}
          title={
            canImplement
              ? "선택한 문제를 구현합니다."
              : "승인되지 않은 건이 포함되어 있습니다. 상세에서 승인 후 실행하세요."
          }
        >
          <Hammer /> 구현 실행
        </Button>
      ...
      <div className="p-4">
        {notes.length === 0 ? (
          <Empty>조건에 맞는 개선 노트가 없습니다.</Empty>
        ) : (
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="w-32">화면</TableHead>
              ...
              {notes.map((n) => {
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell className="truncate text-xs text-muted-foreground" title={n.url}>
                      {n.url || "-"}
                    </TableCell>
                    <TableCell>
      ...
      >
        ...
        {canApprove && (
          <div className="mb-3 rounded-lg border bg-muted/40 p-3">
            <div className="text-xs font-semibold">변경 대상</div>
            <div className="mt-1">
              {changeTargets ? (
                <MarkdownView src={changeTargets} />
              ) : (
                <div className={`text-xs ${WARN_TEXT}`}>
                  `### 변경 대상` 절이 비어 있습니다 — 승인해도 구현되지 않습니다.
                </div>
              )}
            </div>
