...
// 내장 팩의 스킬은 플러그인 루트의 `skills/` 에서 이름으로 찾는다 — Claude Code 플러그인
// 규약이 `skills/` 를 루트에 요구하므로 물리 이동 없이 참조만 한다.
...
fn scan_dir(root: &Path, source: PackSource, skills_root: Option<&Path>) -> (Vec<Pack>, Vec<BrokenPack>) {
    ...
    for dir in dirs {
        ...
        match read_manifest(&manifest_path) {
            Ok(manifest) => {
                // 팩이 자기 스킬을 들고 있으면 그것이 우선. 내장 팩만 플러그인 루트의
                // `skills/` 로 떨어진다 — Claude Code 플러그인 규약이 거기를 요구하므로,
                // SI 팩은 물리 이동 없이 이름으로 참조한다.
                let own = dir.join("skills");
                let skills_dir = match skills_root {
...
pub fn load_registry_from(
    ...
    let (ok, bad) = scan_dir(user_root, PackSource::User, None);
    broken.extend(bad);
...
pub fn render_prompt(template: &str, params: &Map<String, Value>) -> String {
...
mod tests {
    ...
    fn user_pack_overrides_builtin_and_broken_is_reported() {
        ...
        fs::create_dir_all(builtin.join("skills")).unwrap();
    ...
    fn prompt_renders_lists_and_drops_empty_slots() {
        ...
        assert_eq!(
            render_prompt("/sawhorse:issues 설계 {{ids}}", &p),
            "/sawhorse:issues 설계 FDR-1 FDR-2"
        );
        assert_eq!(render_prompt("/x {{missing}} 끝", &Map::new()), "/x 끝");

        let mut nl = Map::new();
        nl.insert("t".into(), serde_json::json!("첫 줄\n둘째 `줄`"));
        assert_eq!(render_prompt("/note {{t}}", &nl), "/note 첫 줄 둘째 줄");

        // 템플릿 자체의 줄바꿈은 살아 있어야 여러 줄 무인 지시를 쓸 수 있다
        let multi = render_prompt("첫 줄 지시\n둘째 줄 {{missing}} 지시", &Map::new());
