...
sawhorse 플러그인의 운영 대시보드(Tauri 2 데스크톱 앱). 볼트를 열지 않고
...
| 화면 | 하는 일 |
|---|---|
| 홈 | 편집 가능한 위젯보드, 루틴 상태, 놓친 예약 알림(자동 보상 실행 없음 — 확인 후 실행), 이슈·작업·볼트 요약 |
...
sawhorse 플러그인이 설치되어 있을 것. herdr 실행을 쓰려면 [herdr](https://herdr.dev)가
설치되고 서버가 떠 있어야 한다(없으면 백그라운드로 자동 폴백).

## 설계 결정 (요약)

- 홈은 위젯 레지스트리와 반응형 그리드 레이아웃을 분리한다. 사용자는 편집 모드에서 위젯의
  위치·크기·표시 여부를 바꿀 수 있고, 레이아웃은 버전이 포함된 로컬 UI 설정
  (`sawhorse.dashboard-layout`)으로 저장된다. 코어 위젯과 배치 모델이 분리되어 있으므로
  이후 외부 위젯 공급자와 권한 모델을 추가해도 보드 렌더러를 교체할 필요가 없다. 보드 컨테이너
  폭에 따라 넓음(12열)·중간(8열)·좁음(4열) 배치를 독립적으로 저장하며, 위젯 내부도 컨테이너
  쿼리로 자신의 실제 폭에 맞춰 다시 배열된다.
- 백엔드가 모든 도메인 로직 소유(Rust), 웹뷰는 뷰만. command + event로 통신.
- 잡 실행기는 둘이고 타임라인 모양은 같다.
  - **herdr** (기본, 가능할 때): 잡마다 `sawhorse` 워크스페이스에 탭을 만들고 그 안에서
