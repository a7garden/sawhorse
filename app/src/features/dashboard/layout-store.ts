...
function isWidgetId(value: unknown): value is DashboardWidgetId {
  return typeof value === "string" && value in WIDGET_BY_ID;
}

function sanitizeItem(item: unknown, breakpoint: DashboardBreakpoint): LayoutItem | null {
  if (!item || typeof item !== "object") return null;
  const value = item as Partial<LayoutItem>;
  if (!isWidgetId(value.i)) return null;
  if (
    ![value.x, value.y, value.w, value.h].every((n) => typeof n === "number" && Number.isFinite(n))
  ) {
    return null;
  }
            ...
            return {
              ...
              h:
                typeof value.h === "number"
                  ? Math.max(1, Math.round((value.h * LEGACY_ROW_PITCH) / ROW_PITCH))
                  : value.h,
            };
          })
        : layout,
...
function sanitizeLayouts(value: unknown): ResponsiveLayouts<DashboardBreakpoint> {
  const fallback = createDefaultLayouts();
  if (!value || typeof value !== "object") return fallback;
  const source = value as Record<string, unknown>;
  const result: ResponsiveLayouts<DashboardBreakpoint> = {};
  for (const breakpoint of Object.keys(DASHBOARD_COLS) as DashboardBreakpoint[]) {
...
function loadDocument(): DashboardLayoutDocument {
  try {
    ...
    if (!parsed || (parsed.version !== 1 && parsed.version !== LAYOUT_VERSION)) {
      return defaultDocument();
    }
    const enabled = Array.isArray(parsed.enabled)
      ? parsed.enabled.filter(isWidgetId)
      : [...DEFAULT_WIDGET_IDS];
    return {
      version: LAYOUT_VERSION,
...
export const useDashboardLayout = create<DashboardLayoutState>((set, get) => ({
  ...
  setLayouts: (layouts) => {
    ...
    saveDocument({ version: LAYOUT_VERSION, enabled: next.enabled, layouts: sanitized });
  },
  setWidgetEnabled: (id, enabled) => {
    ...
    saveDocument({ version: LAYOUT_VERSION, enabled: nextEnabled, layouts: nextLayouts });
  },
  reset: () => {
