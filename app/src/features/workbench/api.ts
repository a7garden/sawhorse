...
/** Validated read of an shdoc/1 HTML artifact (sdd_read_html_document). */
export interface HtmlDocumentView {
  workId: string;
...
export const sddApi = {
  createSamples: (): Promise<import("./samples").SampleReport> => call("sdd_create_samples"),
  ...
  /** Validated shdoc read for .html artifacts; markdown artifacts keep readDocument. */
  readHtmlDocument: (
    workId: string,
    artifact: ArtifactKind,
  ): Promise<HtmlDocumentView> =>
    call("sdd_read_html_document", { workId, artifact }),
