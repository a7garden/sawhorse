...
import { installPreviewSamples } from "./samples";
...
function load(): Store {
  ...
  if (new URLSearchParams(window.location.search).get("shdoc") === "1") {
    snapshot.work.unshift({ ...work("shdoc-demo", "외부 저장소 기여 명세", "spec", "review", "sawhorse", 0), status: "review", artifacts: ["spec"] });
    documents["shdoc-demo/spec"] = {
      workId: "shdoc-demo",
      artifact: "spec",
      path: "work/shdoc-demo/spec.html",
...
<body><main><article id="document">
  ...
  <section id="b-repo-clean" data-sh-kind="requirement">
    ...
    <p>기여 대상 저장소에 개인 workflow 관리 파일을 추가하지 않는다.</p>
  </section>
  <section id="b-evidence" data-sh-kind="acceptance-criterion">
    <h2>승인은 실행 장부에 남는다</h2>
    <p>문서 편집·에이전트 완료·검증·사람 승인을 구분해 기록한다.</p>
  </section>
...
export async function previewInvoke(
  ...
  switch (command) {
    case "sdd_create_samples": {
      const report = installPreviewSamples(state);
      save();
      return report;
    }
    ...
    case "sdd_read_html_document": {
      const d = doc(String(args.workId), String(args.artifact));
      // Browser stand-in only — not the server's shdoc/1 validation. The tour
      // store keeps the HTML source in the Document markdown field, so this
      // just parses it with DOMParser and flags executable/embedding elements.
      const parsed = new DOMParser().parseFromString(d.markdown, "text/html");
      const errors: string[] = [];
      for (const tag of ["script", "iframe", "object", "embed"]) {
        for (const node of parsed.querySelectorAll(tag)) {
          const source = node.getAttribute("src") ?? node.getAttribute("data");
          errors.push(`<${tag}${source ? ` src="${source}"` : ""}>`);
        }
      }
      return {
        workId: d.workId,
        ...
        path: d.path,
        html: d.markdown,
        sourceDigest: d.revision,
        documentId:
          parsed
            .querySelector('meta[name="sawhorse:document-id"]')
            ?.getAttribute("content") ?? "",
        title: parsed.querySelector("title")?.textContent?.trim() ?? "",
        safeForDisplay: errors.length === 0,
        errors,
        warnings: [],
      };
    }
