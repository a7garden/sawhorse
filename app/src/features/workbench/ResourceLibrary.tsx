...
import type { ResourceDocument } from "./lifecycle-v2";
import type { Project } from "./types";
import "./lifecycle.css";
const blank = (kind: "template" | "design"): ResourceDocument => ({ id: "", kind, title: "", markdown: "", source: "", revision: "" });
export function ResourceLibrary({ projects }: { projects: Project[] }) {
  ...
  return <section className="wb-resources" aria-label={t("resources.title")}>
    <header className="wb-header"><div><h1>{t("resources.title")}</h1><p>{t("resources.hint")}</p></div><div className="wb-lifecycle-actions"><Button size="sm" variant="outline" disabled={busy} onClick={() => select(blank("design"))}><Plus />DESIGN.md</Button><Button size="sm" variant="outline" disabled={busy} onClick={() => select(blank("template"))}><Plus />{t("resources.template")}</Button></div></header>
    <div className="wb-resource-layout"><aside className="wb-resource-list">
      ...
      <details><summary>{t("resources.fromDocument")}</summary><p className="wb-lifecycle-notice">{t("resources.sourceHint")}</p>
        <Select aria-label={t("resources.sourceProject")} value={projectId} disabled={busy} onChange={setProjectId} options={[{ value: "", label: t("resources.chooseSourceProject") }, ...projects.map((p) => ({ value: p.id, label: p.name }))]} />
      ...
      <Button size="sm" disabled={busy || !source.trim() || !(project?.defaultAgent || defaultAgent)} onClick={() => void perform(async () => { update({ markdown: await sddApi.generateResource(draft.kind, source, project?.defaultAgent || defaultAgent) }); })}>{busy ? <Loader2 className="wb-spin" /> : <Sparkles />}{t("resources.generate")}</Button></details>
