...
function desktopCall<T>(command: string, args?: Record<string, unknown>): Promise<T> {
  if (!isTauri()) {
    return Promise.reject(
      new Error("스키마 검사와 파일 적용은 Sawhorse 데스크톱 앱에서 사용할 수 있습니다."),
    );
  }
...
export const schemaStudioApi = {
  ...
  plan: (schema: VaultSchema, scan: SchemaScanResult): Promise<SchemaMigrationPlan> =>
  ...
  apply: (id: string): Promise<ChangeSet> => desktopCall("changeset_apply", { id }),
  ...
  saveDraft: (draftId: string, schema: VaultSchema): Promise<SchemaDraftRecord> =>
  ...
  activate: (schema: VaultSchema, changeSetId?: string): Promise<ActiveSchemaState> =>
    desktopCall("schema_activate", { schema, changeSetId: changeSetId ?? null }),
