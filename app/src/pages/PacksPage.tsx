              ...
              <Card>
                ...
                <CardContent className="space-y-2">
                  <p className="text-[11px] leading-relaxed text-muted-foreground">
                    Claude Code 에는 번들 플러그인을 ~/.claude/skills/sawhorse 로 통째로 펼칩니다
                    (skills-dir 플러그인 — 설치 단계 없이 자동 적재). 손으로 고친 파일은 덮지 않고
                  ...
                  {status?.pluginInstalls && status.pluginInstalls.length > 0 && (
                    <div className="rounded-md border border-warning/40 bg-warning/10 px-2 py-1.5 text-[11px]">
                      ...
                      같은 내용이므로 앱은 사본을 만들지 않습니다 — 스킬은 그 플러그인이 제공합니다.
                    </div>
                  )}
                        ...
                        <Button
                          ...
                          title={
                            a.params.some((p) => p.required)
                              ...
                              : a.prompt.replace(
                                  "{{ns}}",
                                  sel.source === "builtin" ? "sawhorse" : `sawhorse-${sel.id}`,
                                )
                          }
