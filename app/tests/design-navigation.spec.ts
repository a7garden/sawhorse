...
test("the default desk prioritizes work, and new intents inherit the project", async ({ page }) => {
  ...
  await expect(today.getByRole("heading", { name: "오늘 활동" })).toBeInViewport();
  const todayBounds = (await today.boundingBox())!;
  const jobsBounds = (await jobs.boundingBox())!;
  expect(Math.abs(todayBounds.y - jobsBounds.y)).toBeLessThan(2);
  expect(todayBounds.width).toBeGreaterThan(jobsBounds.width);
...
test("collapsed navigation persists and the project menu remains readable", async ({ page }) => {
  ...
  const menu = page.getByRole("listbox", { name: "프로젝트 전환", exact: true });
  ...
  await expect(scope(page)).toHaveAttribute("aria-label", "프로젝트 전환");
...
test("existing customized dashboards keep their widgets and dimensions", async ({ page }) => {
  await page.addInitScript(() => {
    localStorage.setItem("sawhorse.dashboard-layout", JSON.stringify({
      version: 4,
      enabled: ["projects", "metric:done"],
      layouts: {
        lg: [{ i: "projects", x: 0, y: 0, w: 7, h: 10 }, { i: "metric:done", x: 7, y: 0, w: 5, h: 5 }],
        md: [{ i: "projects", x: 0, y: 0, w: 8, h: 10 }, { i: "metric:done", x: 0, y: 10, w: 4, h: 5 }],
        sm: [{ i: "projects", x: 0, y: 0, w: 4, h: 10 }, { i: "metric:done", x: 0, y: 10, w: 4, h: 5 }],
      },
    }));
  });
  await page.goto("/?preview=1");
  await expect(page.locator(".widget-grid-item")).toHaveCount(2);
  await expect(page.getByRole("heading", { name: "프로젝트 현황", exact: true })).toBeVisible();
  const saved = await page.evaluate(() => JSON.parse(localStorage.getItem("sawhorse.dashboard-layout")!));
  expect(saved.enabled).toEqual(["projects", "metric:done"]);
  expect(saved.layouts.lg.find((item: { i: string }) => item.i === "projects")).toMatchObject({ w: 7, h: 10 });
});
