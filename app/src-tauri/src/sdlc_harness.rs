...
fn spawn_start(root: PathBuf, id: String) {
    ...
    let Ok(owner) = crate::workspace_io::lock(&root, &format!("run-owner-{id}")) else {
        return;
    };
...
fn launch_gate(root: &Path, input: &LaunchInput) -> Result<LaunchContext, String> {
    ...
    for peer in list_records(root)?
        .into_iter()
        .filter(|r| refreshable_status(&r.status) && r.work_id != work.id)
    {
        if (peer.workflow_id == sdlc::goals::WORKFLOW || work.workflow_id == sdlc::goals::WORKFLOW)
            && Path::new(&peer.repo_path).canonicalize().ok()
                == Path::new(&project.repo_path).canonicalize().ok()
        {
            let scope = sdlc::goals::read(root, &work.id)
                .map(|s| s.scope)
                .unwrap_or_else(|_| vec![".".into()]);
            let peer_scope = sdlc::goals::read(root, &peer.work_id)
                .map(|s| s.scope)
                .unwrap_or_else(|_| vec![".".into()]);
            if sdlc::goals::scopes_overlap(&scope, &peer_scope) {
                return Err(format!(
                    "변경 범위가 겹치는 작업 {}이 점유 중입니다 (실행 {})",
                    peer.work_id, peer.id
                ));
            }
        }
    }
    ...
    if input.parent_run_id.is_none() {
        if let Some(running) = list_records(root)?
            .into_iter()
            .find(|record| record.work_id == input.work_id && refreshable_status(&record.status))
        {
            return Err(format!(
    ...
    for artifact in &node.inputs {
        ...
        if !sdlc::substantial_for(&document.path, &document.markdown) {
            return Err(format!(
...
fn record_launch_with_config(
    ...
    if sdlc::lifecycle::supports(work) && matches!(work.stage.as_str(), "build" | "discarding") {
        let common = sdlc::lifecycle::git(
            Path::new(&context.repo_path),
            &["rev-parse", "--path-format=absolute", "--git-common-dir"],
        )?;
        if !context.extra_paths.contains(&common) {
            context.extra_paths.push(common);
        }
    }
    ...
    // v2 설계 §8: 실행 시점의 필수 산출물을 정확한 revision으로 고정한다.
    // 필수 입력을 읽지 못하면 실행을 시작하지 않는다(누락은 차단, 참고 부재와 다름).
    let mandatory = context
        ...
        .collect::<Result<Vec<_>, _>>()?;
    let snapshot = crate::documents::context::capture_at(root, &work.id, &record.id, &mandatory)?;
    crate::documents::context::record_at(root, &snapshot)?;
...
/// herdr 오류 중 일시적(timeout, 스폰 실패류)인 것을 가린다. HerdrError는
/// "<message> (<code>)"로 표시되므로 괄호 안 코드로 판정한다. not_found나
/// 신원 불일치는 이 목록에 없어 최종 실패로 남는다.
fn herdr_error_is_transient(error: &str) -> bool {
    ["(timeout)", "(spawn_failed)", "(spawn_not_found)"]
        .iter()
...
async fn start_record(root: PathBuf, id: String) {
        ...
        .await
    {
        Ok(tab) => tab,
        Err(error) => {
            // 탭 생성에 실패하면 이 런을 위해 만든 워크스페이스가 herdr에
            // 빈 채로 남는다 — 기록하기 전에 닫는다.
            let _ = h.close_workspace(&workspace).await;
            update(
                &mut record,
                "failed",
...
async fn refresh_record_with(
    ...
    let info = match owned_agent(h, &record).await {
        Ok(info) => info,
        // herdr CLI/server의 일시 실패(timeout, 스폰 실패)는 런을 실패로 확정할
        // 근거가 아니다 — 이번 틱은 건너뛰고 다음 갱신에 맡긴다. not_found나
        // 신원 불일치만 최종 실패다.
        Err(error) if herdr_error_is_transient(&error) => return Ok(record),
        Err(error) => {
            update(
                &mut record,
                "failed",
...
pub async fn sdd_continue_run(id: String, instructions: String) -> Result<HarnessRun, String> {
    ...
    if list_records(&root)?
        .iter()
        .any(|run| run.work_id == record.work_id && run.id != id && refreshable_status(&run.status))
    {
    ...
    if record.runner == "headless" {
        let _launch_guard = launch_mutex()
            .lock()
            .map_err(|_| "harness launch lock이 손상되었습니다".to_string())?;
...
pub async fn sdd_work_copilot(input: CopilotAsk) -> Result<CopilotAnswer, String> {
    ...
    if question.chars().count() > COPILOT_MAX_QUESTION {
        return Err(format!(
            "질문은 {COPILOT_MAX_QUESTION}자를 넘을 수 없습니다"
        ));
    }
    let root = sdlc::vault_root()?;
    sdlc::validate_id(&input.work_id)?;
...
mod tests {
    ...
    fn dismiss_hides_a_settled_run_without_losing_it() {
        ...
        assert!(dismiss_record(&root, &failed.id, false)
            ...
            .dismissed_at
            .is_none());
        assert!(load_record(&root, &failed.id)
            .unwrap()
            .dismissed_at
            .is_none());
    }
    ...
    fn dismiss_rejects_a_run_that_is_still_going() {
        ...
        assert!(dismiss_record(&root, &running.id, true)
            .unwrap_err()
            .contains("진행 중인 실행"));
        assert!(load_record(&root, &running.id)
            .unwrap()
            .dismissed_at
            .is_none());
