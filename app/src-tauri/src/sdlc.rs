...
pub(crate) fn work_type_guidance(work: &WorkItem) -> String {
    ...
    format!(
        "작업 유형: {}\n{}\n단계와 승인 절차는 고정된 워크플로우를 따릅니다.",
        work.issue_type, guidance
    )
}
...
fn normalize_project_workflow(project: &mut Project) {
    ...
    project
        ...
        .retain(|reference| workflow::builtins::is_selectable(&reference.id));
...
fn normalize_work_workflow(work: &mut WorkItem) {
    ...
    if let Some(value) = work
        .issue_type
        .is_ascii()
        .then(|| canonical_issue_type(&work.issue_type))
        .flatten()
    {
    ...
    if let Some(value) = work
        .execution_type
        .is_ascii()
        .then(|| canonical_execution_type(&work.execution_type))
        .flatten()
    {
...
/// The configured vault root, resolved through the default document space.
/// Kept public for the harness module and tests.
///
/// v2 R1A: `.sawhorse/spaces.json`이 있으면 문서 공간 설정의 기본 공간 루트를
/// 따르고, 없으면 설정 경로를 그대로 default DocumentStore로 쓴다(동작 보존).
pub fn vault_root() -> Result<PathBuf, String> {
    ...
    if crate::document_spaces::spaces_config_path(&configured).is_file() {
        return crate::document_spaces::resolve_root(&configured, None);
    }
    Ok(configured)
}
...
/// 산출물 형식에 맞는 실질성 검사. HTML 산출물은 태그를 벗긴 가시 텍스트
/// 기준으로 판정한다(v2 설계 §5). 나머지는 기존 `substantial`과 같다.
pub(crate) fn substantial_for(path: &str, contents: &str) -> bool {
    if documents::service::is_html_artifact_path(Path::new(path)) {
        documents::service::substantial_html(contents)
    } else {
...
fn existing_artifacts(
    ...
    definition
        ...
        .filter_map(|artifact| {
            ...
            substantial_for(&p.to_string_lossy(), &contents).then(|| artifact.role.clone())
        })
        .collect()
}
...
pub fn save_project_at(root: &Path, mut input: Project) -> Result<Project, String> {
    ...
    input
        .additional_workflows
        .retain(|reference| enabled.insert((reference.id.clone(), reference.version.clone())));
    for reference in &input.additional_workflows {
...
pub fn activate_project_workflow_checked_at(
    ...
    let old_default = workflow::WorkflowRef {
        id: previous.workflow_id,
        version: previous.workflow_version,
    };
    ...
    previous
        .additional_workflows
        .retain(|reference| reference.id != workflow_id || reference.version != workflow_version);
    header.insert(
        serde_yaml::Value::String("additionalWorkflows".into()),
...
fn bind_project_workflow(root: &Path, input: &mut WorkItem) -> Result<(), String> {
    ...
    if !(input.workflow_id == project.workflow_id
...
fn save_work_locked(root: &Path, mut input: WorkItem) -> Result<WorkItem, String> {
    ...
    if let Some(old) = &previous {
        if old.workflow_id == goals::WORKFLOW
            && (old.project_id != input.project_id || old.description != input.description)
        {
...
fn write_document_at(
    ...
    // v2 설계 §6: HTML 산출물 저장 시점의 표현 근거를 DesignBinding으로 남긴다.
    // 프로젝트에 적용된 DESIGN.md가 없으면 기록하지 않는다(무스타일 읽기).
    if documents::service::is_html_artifact_path(&path) {
        if let Some(binding) = documents::design::resolve_for_work(root, work_id)? {
            documents::design::bind_at(root, work_id, &binding)?;
        }
    }
    work.artifacts = existing_artifacts(root, &work, &definition);
    work.updated_at = now();
...
fn ensure_stage_gate(
    ...
    for artifact in &node.inputs {
        ...
        if !substantial_for(&doc.path, &doc.markdown) {
            return Err(format!(
...
fn work_lifecycle_at(
    ...
    if matches!(action, "submit" | "complete") {
        ...
        for role in node.inputs.iter().chain(&node.outputs) {
            let doc = read_document(root, &work.id, role)?;
            if !substantial_for(&doc.path, &doc.markdown) {
                return Err(format!(
...
fn capture_intent_with_scope_at(
    root: &Path,
    mut input: CaptureIntentInput,
    project_scoped: bool,
) -> Result<WorkItem, String> {
    use base64::Engine;
    let _guard = mutation_lock();
...
/// shdoc HTML 산출물의 읽기 뷰 — 원문과 파싱·검증 진단 (v2 설계 §5·§14).
/// 원문은 `sdd_read_document`와 같은 bytes고, 여기엩 표시 안전성 진단이 붙는다.
#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
...
#[tauri::command]
pub fn sdd_read_html_document(
    work_id: String,
    artifact: String,
) -> Result<HtmlDocumentView, String> {
    let root = vault_root()?;
    let doc = read_document(&root, &work_id, &artifact)?;
    let absolute = root.join(&doc.path);
    let mut warning_extra = None;
    let view = if absolute.is_file() {
        documents::service::read_html_at(&absolute)?
    } else {
        // 아직 저장되지 않은 산출물: 워크플로 템플릿을 원문 후보로 보여준다.
        warning_extra = Some("아직 저장되지 않은 템플릿입니다.".to_string());
        let parsed = documents::codec::parse_native_html(&doc.markdown).ok();
        let mut validation = match &parsed {
            Some(p) => documents::validation::validate(&doc.markdown, p),
            None => documents::validation::ShdocValidation::default(),
        };
        if parsed.is_none() {
            validation
                .errors
                .push("HTML 파싱 실패: 템플릿이 shdoc/1 계약을 따르지 않습니다".into());
        }
        documents::service::HtmlDocument {
            html: doc.markdown.clone(),
            source_digest: documents::codec::source_digest(doc.markdown.as_bytes()),
            parsed,
            validation,
        }
    };
    let (document_id, title) = view
        .parsed
        .as_ref()
        .map(|parsed| (parsed.document_id.clone(), parsed.title.clone()))
        .unwrap_or_default();
    let mut warnings = view.validation.warnings;
    if let Some(extra) = warning_extra {
    ...
    Ok(HtmlDocumentView {
        ...
        document_id,
        title,
        safe_for_display: view.parsed.is_some() && view.validation.errors.is_empty(),
...
mod tests {
    ...
    fn work_purpose_is_independent_of_process_and_legacy_pins_survive() {
        ...
        for id in ["bugfix-main", "refactor-main"] {
            ...
            old.workflow_id = id.into();
            old.workflow_version = "1.0.0".into();
            let old = save_work_at(&root, old).unwrap();
            fs::write(
                project_path(&root, "p"),
                markdown(&p, "Existing project").unwrap(),
            )
            .unwrap();
            ...
            forbidden.workflow_id = id.into();
            forbidden.workflow_version = "1.0.0".into();
            assert!(save_project_work_at(&root, forbidden).is_err());
            assert_eq!(
                save_project_work_at(&root, old.clone())
                    .unwrap()
                    .workflow_digest,
                old.workflow_digest
            );
            assert_eq!(
                workflow::resolve(Some(&root), &old.workflow_id, &old.workflow_version)
                    .unwrap()
                    .id,
                id
            );
    ...
    fn renamed_intent_and_goal_bundles_keep_the_project_pin_when_creating_work() {
        ...
        for version in ["2.0.0", "2.0.1"] {
            ...
            let saved = capture_intent_with_scope_at(
                &root,
                CaptureIntentInput {
                    work: item,
                    markdown: "A clear outcome".into(),
                    attachments: vec![],
                },
                true,
            )
        ...
        for version in ["1.0.0", "1.0.1"] {
            ...
            let saved = goals::create_at(
                &root,
                goals::CreateGoal {
                    id: format!("goal-{}", version.replace('.', "-")),
                    project_id: "p".into(),
                    objective: "A clear outcome".into(),
                    max_parallel: 1,
                    start: false,
    ...
    fn project_creation_inherits_exact_pin_and_preserves_existing_work_after_settings_change() {
        ...
        assert_eq!(
            save_project_work_at(&root, wrong).unwrap().workflow_id,
            "intent-flow"
        );
        fs::remove_dir_all(root).unwrap();
    }
    ...
    fn project_supports_multiple_exact_flows_and_removal_preserves_existing_work() {
        ...
        assert_eq!(
            project_by_id(&root, "p").unwrap().additional_workflows,
            saved_project.additional_workflows
        );
        ...
        let captured = capture_intent_with_scope_at(
            &root,
            CaptureIntentInput {
                work: intent,
                markdown: "An idea in a mixed project".into(),
                attachments: vec![],
            },
            true,
        )
        ...
        let goal = goals::create_at(
            ...
            goals::CreateGoal {
                ...
                max_parallel: 1,
                start: false,
                workflow_version: "1.0.1".into(),
                issue_type: "리팩토링".into(),
            },
        )
        .unwrap();
        ...
        let mut p = project_by_id(&root, "p").unwrap();
        p.additional_workflows.push(workflow::WorkflowRef {
            id: "tdd-cycle".into(),
            version: "9.0.0".into(),
        });
        assert!(save_project_at(&root, p).is_err());
        assert!(project_by_id(&root, "p")
            .unwrap()
            .additional_workflows
            .is_empty());
        fs::remove_dir_all(root).unwrap();
    ...
    fn project_intent_creation_rejects_changed_settings_without_writes_and_allows_saved_retries() {
        ...
        assert_eq!(
            read_document(&root, "capture", "intent").unwrap().markdown,
            "Keep this draft"
        );
        fs::remove_dir_all(root).unwrap();
    }
    fn capture_legacy_intent(root: &Path, input: CaptureIntentInput) -> Result<WorkItem, String> {
        ...
        work.workflow_version = "1.0.0".into();
        work.stage = "design".into();
    ...
    #[test]
    fn html_artifact_vertical_flow_from_write_to_snapshot() {
        let root = tempdir("html-vertical");
        initialize(&root).unwrap();
        save_project_at(&root, project("p")).unwrap();

        // 새 HTML 정본은 새 워크플로 버전에서만 선언한다(v2 설계 §9).
        let template = "<!doctype html>\n<html lang=\"ko\">\n<head>\n  <meta charset=\"utf-8\">\n  <meta name=\"sawhorse:format\" content=\"shdoc/1\">\n  <meta name=\"sawhorse:document-id\" content=\"doc-template\">\n  <title>요청</title>\n</head>\n<body><main><article id=\"document\">\n  <h1>요청</h1>\n</article></main></body></html>\n";
        let mut definition = workflow::builtins::issue();
        definition.id = "html-flow".into();
        definition.label = "HTML 수직 흐름 (테스트)".into();
        definition.version = "1.0.0".into();
        definition.artifacts[0].path = "work/{workId}/intent.html".into();
        definition.artifacts[0].template = template.into();
        workflow::publish_at(&root, definition).unwrap();
        activate_project_workflow_at(&root, "p", "html-flow", "1.0.0").unwrap();

        let mut input = work("h1", "p");
        input.stage = "request".into();
        let item = save_project_work_at(&root, input).unwrap();
        assert_eq!(item.workflow_id, "html-flow");

        // 1) 저장 전에는 워크플로 템플릿이 원문 후보로 내려온다.
        let doc = read_document(&root, "h1", "intent").unwrap();
        assert!(doc.path.ends_with(".html"), "path={}", doc.path);
        assert!(doc.markdown.contains("shdoc/1"));

        // 2) HTML 원문 저장 — 개정은 원문 bytes의 해시고 경로는 그대로다.
        let html = "<!doctype html>\n<html lang=\"ko\">\n<head>\n  <meta charset=\"utf-8\">\n  <meta name=\"sawhorse:format\" content=\"shdoc/1\">\n  <meta name=\"sawhorse:document-id\" content=\"doc-h1-intent\">\n  <title>외부 기여 요청</title>\n</head>\n<body><main><article id=\"document\">\n  <h1>외부 기여 요청</h1>\n  <section id=\"b-goal\" data-sh-kind=\"goal\">\n    <h2>목표</h2>\n    <p>개인 방식으로 기여하고 저장소에는 관리 파일을 남기지 않는다.</p>\n  </section>\n</article></main></body></html>\n";
        let saved = write_document_at(&root, "h1", "intent", html.into(), &doc.revision).unwrap();
        assert_eq!(saved.revision, revision(html));
        assert!(root.join("work/h1/intent.html").is_file());

        // 3) 오래된 개정으로 다시 저장하면 기존 충돌 계약이 그대로 막는다.
        let conflict =
            write_document_at(&root, "h1", "intent", html.into(), &doc.revision).unwrap_err();
        assert!(conflict.contains("다른 편집으로 변경"));

        // 4) 읽기 뷰 — 파싱·검증 진단이 원문과 함께 돌아온다.
        let view = documents::service::read_html_at(&root.join("work/h1/intent.html")).unwrap();
        assert!(view.parsed.is_some());
        assert!(view.validation.errors.is_empty());
        assert_eq!(view.source_digest, revision(html));
        assert_eq!(view.parsed.as_ref().unwrap().document_id, "doc-h1-intent");

        // 5) 게이트 실질성은 형식을 가린다. HTML은 가시 텍스트 기준.
        assert!(substantial_for("work/h1/intent.html", html));
        assert!(!substantial_for(
            "work/h1/intent.html",
            "<section><!-- 메모만 --></section>\n"
        ));
        assert!(substantial_for(
        ...
        // 6) 프로젝트에 DESIGN.md를 적용하면 HTML 저장 시 바인딩이 남는다(§6).
        fs::create_dir_all(root.join("projects/p")).unwrap();
        fs::write(
            root.join("projects/p/DESIGN.md"),
            "# 디자인 지침\n\n차분한 톤, 13px 본문.\n",
        )
        .unwrap();
        fs::write(
            root.join("projects/p/resources.json"),
            "{\"designId\":\"d-1\",\"templates\":{}}",
        )
        .unwrap();
        let doc2 = read_document(&root, "h1", "intent").unwrap();
        write_document_at(&root, "h1", "intent", html.into(), &doc2.revision).unwrap();
        let binding = documents::design::binding_at(&root, "h1").unwrap().unwrap();
        assert_eq!(binding.purpose, "document-view");
        assert!(binding.revision.len() == 64);

        // 7) 실행 입력 고정 — 필수 산출물의 revision 스냅샷이 runs/에 남는다(§8).
        let snapshot = documents::context::capture_at(
            &root,
            "h1",
            "run-1",
            &[root.join("work/h1/intent.html")],
        )
        .unwrap();
        assert_eq!(snapshot.mandatory.len(), 1);
        assert_eq!(snapshot.mandatory[0].revision, revision(html));
        let recorded = documents::context::record_at(&root, &snapshot).unwrap();
        assert!(recorded.ends_with("runs/run-1.context.json"));
        assert!(root.join("runs/run-1.context.json").is_file());
        fs::remove_dir_all(root).unwrap();
    }
}
