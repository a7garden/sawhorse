...
5. **볼트 위생 (scan)** — 아래 [HYGIENE] 계약대로 `scan` 모드를 실행한다. `scan`은 어떤 파일도 쓰지 않으므로 이 스킬의 읽기 전용 원칙과 충돌하지 않는다. **진단 결과를 여기서 고치지 않는다** — 목록만 결산에 싣고, 수정은 evening(`fix`)이나 `/sawhorse:vault-tidy`가 한다.
...
- `[파생표]` — 이슈 MOC·이슈목록이 이슈 프로퍼티를 마크다운 표로 베껴 두고 있다. **반드시 어긋나므로** `.base` 뷰 임베드로 바꿔야 한다. 본문을 다시 쓰는 일이라 여기서 자동으로 고치지 않고, 목록과 함께 `/sawhorse:issues` 또는 `/sawhorse:vault-tidy` 를 안내한다.
- `[충돌]` `[경고]` — 그대로 보고한다.
