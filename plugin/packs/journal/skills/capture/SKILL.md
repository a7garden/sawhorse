...
고쳐 쓰지 않는다. 이 스킬은 **일지(journal) 확장**의 것이고 업종 어휘를 쓰지 않는다.
...
- `vaultPath` — 작업공간 루트. 비어 있으면 **아무것도 쓰지 않고** 대시보드에서 작업공간을
  먼저 지정하라고 안내하고 멈춘다.
- `packs.settings.journal.useWikiLinks` — 참이면 주제어를 `[[문서 이름]]` 으로 잇는다.
- `packs.settings.journal.ownerName` — 사람 이름이 필요할 때만 쓴다.

## 절차

1. 오늘 날짜(`YYYY-MM-DD`)로 `<작업공간>/일지/<날짜>.md` 경로를 정한다. 기존 작업공간에 `일지/`는 없고 `기록/`만 있으면 기존 폴더를 계속 사용한다.
2. 파일이 없으면 `<작업공간>/템플릿/journal/일지.md`를 복사해 만든다. 없으면 기존 `템플릿/일지.md` 또는 `템플릿/기록.md`를 사용한다. 템플릿이 없으면 아래 최소
...
기존 일지는 해당 문서의 절을 유지한다. 기존 `할 일`은 `오늘 할 일`, `한 일`은 `업무기록`, `메모`는 `비고`에 대응하며 중복 절을 새로 만들지 않는다.

3. `--today` 인 경우: 어제 일지의 `## 내일 할 일` 또는 기존 `## 할 일` 에서 **체크되지 않은 줄만** 오늘 `## 오늘 할 일` 로
...
4. 내용이 있는 경우: 문장을 읽고 자리를 정해 **줄 하나를 덧붙인다**.
   ...
   | 문장 | 자리 |
   |---|---|
| "~할 것", "~해야 함", 미래 시제 | `## 오늘 할 일` 에 `- [ ] <문장>` |
| "~했다", "~완료", 과거 시제 | `## 업무기록` 에 `- <문장>` |
| 그 밖에 (사실·아이디어·링크) | `## 비고` 에 `- <문장>` |

   판단이 애매하면 `## 비고` 에 원문 그대로 넣는다. **분류를 위해 문장을 고치지 않는다.**
5. `useWikiLinks` 가 참이면, 방금 넣은 줄에서 반복 등장하는 주제어 하나를 `[[주제]]` 로
   감싸고 `<작업공간>/문서/<주제>.md` 가 없으면 `템플릿/journal/문서.md`로 만든다(없으면 기존 `템플릿/문서.md` 참고)(죽은 링크 금지). 기존 작업공간에 `문서/`는 없고 `노트/`만 있으면 기존 경로와 템플릿을 계속 사용한다.
