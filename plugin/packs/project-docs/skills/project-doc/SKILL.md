...
# 프로젝트 문서화 확장 (project-doc)

앱의 프로젝트 등록은 `projects/<id>/project.md`에서 관리한다. 이 스킬은 별도의 프로젝트나 이슈 저장소를 만들지 않는다.

제안서·프로젝트 문서를 vault로 가져와 `프로젝트/<프로젝트명>/` 폴더, 프로젝트 허브(MOC), 제안 요약을 만든다. 개념 노트는 `concepts` 확장이 활성화된 경우에만 함께 만든다.
...
`$ARGUMENTS` = 프로젝트명 [제안서/문서 경로들...] [코드베이스 경로]

- 프로젝트명: vault 폴더명이 된다(예: `스마트교통관리시스템고도화`).
...
1. **인자 파싱**
   - 프로젝트명이 없으면 사용자에게 문의하고 중단한다.
   - 프로젝트명은 vault 폴더명이 된다. Windows 금지 문자(`\` `/` `:` `*` `?` `"` `<` `>` `|`)는 제거하거나 `-`로 치환한 폴더명을 쓰고, 결과 폴더명을 결과 보고에 남긴다.
...
2. **프로젝트 폴더 생성**
   ...
   - `concepts` 확장이 켜진 경우에만 필요 시 `${user_config.vault_path}/개념/`도 만든다.
...
4. **허브 문서 작성** — `프로젝트/<프로젝트명>/<프로젝트명>.md`
   - `${user_config.vault_path}/템플릿/프로젝트.md` 내용으로 노트를 만들고 frontmatter를 채운다(스키마 순서 고정):
     ...
     period: <문서에서 프로젝트기간 추출, 못 찾으면 빈 값>
     parent:
     related: []
   ...
   - 발주처/프로젝트기간/주요 요구사항은 추출 원문(발주처, 기관명, 프로젝트기간, 수행기간, 기능요건 등 키워드)에서 근거를 찾는다. 확실하지 않으면 빈 값으로 두고 7단계에서 사용자 확인을 요청한다.
   - `## 개요`에 프로젝트 목적 1-3문장, `## 문서`에 `[제안 요약](산출물/제안요약.md)` 링크, `## 주요 개념`에 `[[개념명]]` 목록을 채운다.
   - 허브가 이미 존재하면 덮어쓰지 않고 `## 문서`/`## 주요 개념`에 링크를 추가(중복 제거)만 한다.

5. **제안 요약 작성** — `산출물/제안요약.md`
   - frontmatter 없이(스키마에 없는 노트이므로) 마크다운 본문만: 프로젝트 개요 / 프로젝트 범위 / 주요 요구사항 요약.
   - 각 항목 아래에 출처를 표기한다: `> 출처: <문서파일명> "<챕터·헤딩명>"`. 원문이 vault 밖이면 링크 대신 이 텍스트 출처로 대체한다.
   - 본문에서 도메인 용어는 `[[개념명]]`으로 표기해 둔다(6단계에서 노트 생성).
   - 시각화 우선: 시스템 구성·연동 관계는 mermaid `flowchart`로, 핵심 처리 흐름은 `sequenceDiagram`으로 제안 요약에 포함한다. mermaid로 표현이 복잡한 전체 아키텍처 그림은 SVG로 직접 그려 `${user_config.vault_path}/첨부/다이어그램/<프로젝트명>/`에 저장하고 요약에서 임베드한다.

6. **개념 노트 생성·링크 (`concepts` 확장이 켜진 경우만)**
...
7. **결과 보고**
   ...
   - 다음 단계 안내: `/sawhorse:issues 등록 <프로젝트명>` 및 (코드베이스가 있으면) `/sawhorse:codebase-docs <프로젝트명> <코드베이스 경로>`.
