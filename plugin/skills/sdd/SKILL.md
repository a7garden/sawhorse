...
Apply the sibling [delegate skill](../delegate/SKILL.md) automatically when decomposing work. Assess each bounded subtask yourself and submit its complexity and reason; leave its model empty for the host policy. The harness also embeds this policy so it applies when skills are not separately installed. Tiny tasks may be cheaper to finish locally. Children write evidence to their own run evidence file; the parent owns canonical artifacts.
