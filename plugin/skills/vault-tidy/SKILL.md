---
name: vault-tidy
description: Inspect and tidy journal and knowledge documents, root attachments, and document links. Use for explicit vault cleanup or as the document-maintenance step of a Sawhorse routine.
---

# 문서 정리

일지·개념·프로젝트 지식 문서를 정리한다. 앱의 작업 데이터나 확장 산출물을 특정 기능 확장의 템플릿으로 정규화하지 않는다.
...
- 스키마: 일반 지식 문서는 볼트에 있는 해당 템플릿을 참고한다. 사용자 필드를 삭제하지 않는다. 코어 `.sawhorse/schema.json`과 사용자 문서 스키마 `.sawhorse/schemas/`는 특정 확장의 템플릿으로 대체하지 않는다.
