...
새 의도의 기본 입력은 제목·담당자·기한 양식 대신 자유로운 Markdown 메모와 이미지다. 기존 Atomic Editor로 라이브 편집하고 미리보기를 전환한다. 클립보드 이미지, 브라우저 파일 드롭, 데스크톱의 네이티브 파일 드롭과 파일 선택을 지원한다. 이미지는 별도 첨부 목록 대신 커서·드롭 위치의 본문 블록으로 삽입한다. 미리보기와 실행 취소·다시 실행도 같은 Markdown을 사용한다. 메모 없이 이미지로만 시작할 수도 있다. 제목은 첫 문장에서 만들며 프로젝트를 선택하기 전에도 메모를 저장할 수 있다.

이 입력은 `intent-flow@1.0.0`으로 저장한다. 프로젝트에 고정된 기존 SDD/TDD 항목은 유지한다. 원문은 `intent.md`, 이미지 파일은 같은 작업 폴더의 `attachments/`에 내용 해시 이름으로 저장하고, 본문의 이미지 임베드 위치는 유지한다. 임시 이미지 URL은 저장할 때 상대 경로로 치환하며 본문에서 삭제한 이미지는 저장하지 않는다. 에이전트는 원문과 이미지를 읽고 `spec.md`에 설계·수용 기준·가정·열린 질문을, `plan.md`에 ID·범위·의존성·검증 방법이 있는 작업 단위를 작성한다. 작업 단위는 계획 문서의 체크리스트이며 별도 WorkItem을 자동 생성하지 않는다.
