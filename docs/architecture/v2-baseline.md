...
위 Playwright 판정의 정리는 커밋 `07a69a7`으로 착수했다. 네비 재편·워크벤치 스코프화에 맞춰 스펙 19개 파일을 실제 UI에 맞게 고쳤고, 검증 중 발견한 앱 결함 2건(preview 시드 미영속화, 실행 오류 메시지가 폴러에 의해 즉시 지워짐)을 근원부에서 수정했다. 전체 스위트 154개 통과(0 실패).
