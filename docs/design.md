...
<VAULT>/                            # ${user_config.vault_path}
  ...
  일지/일지.base                    # 최근 일지 뷰
  사업/사업.base                    # 진행중 / 전체 뷰
  사업/이슈.base                    # 볼트 전체 이슈 뷰 (대시보드용, 사업을 가로지름)
  사업/마일스톤.base                # 볼트 전체 마일스톤 뷰
  사업/<사업명>/
    <사업명>.md                     # 사업 허브(MOC)
    ...
    이슈/                           # 이슈 산출물 (평면 — 화면별 하위 폴더를 두지 않는다)
      ...
      <idPrefix> 이슈.base          # 이 사업 범위의 이슈 뷰
  ...
  첨부/목업/<사업명>/               # 코드 이슈의 UI 목업 HTML (승인 전 화면 확인용)
```
...
- Bases 임베드는 **필터를 덧붙일 수 없다.** 그래서 범위마다 `.base` 파일이 하나씩 필요하다(볼트 전체 / 사업). 뷰 정의가 몇 벌 늘어나는 것은 데이터가 두 벌 되는 것과 다른 문제다 — 뷰는 어긋나도 열이 달라질 뿐이고, 데이터는 거짓말을 한다.
- `.base` 이름은 볼트 안에서 유일해야 한다(위키링크·임베드가 이름으로 해석된다). 사업 폴더의 것은 `<idPrefix> 이슈.base`, 인박스는 `<idPrefix> 이슈목록.md`.
