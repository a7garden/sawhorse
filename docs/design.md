...
<VAULT>/                            # ${user_config.vault_path}
  ...
  일지/일지.base                    # 최근 일지 뷰
  프로젝트/프로젝트.base                    # 진행중 / 전체 뷰
  프로젝트/이슈.base                    # 볼트 전체 이슈 뷰 (대시보드용, 프로젝트을 가로지름)
  프로젝트/마일스톤.base                # 볼트 전체 마일스톤 뷰
  프로젝트/<프로젝트명>/
    <프로젝트명>.md                     # 프로젝트 허브(MOC)
    분석/                           # codebase-docs 산출물 (기능 1개 = 문서 1개)
    산출물/
    회의/
    이슈/                           # 이슈 산출물 (평면 — 화면별 하위 폴더를 두지 않는다)
      이슈.md                       # MOC — base 뷰 임베드 + 교차검토·순서·배치 설명
      <idPrefix> 이슈.base          # 이 프로젝트 범위의 이슈 뷰
      <idPrefix> 이슈목록.md         # 사용자가 자유 작성하는 인박스 + 화면별 뷰 임베드
      <ID> <제목>.md                # 이슈 노트 (배경·설계·결과 한 노트)
    마일스톤/                       # 납기·릴리스·검수 목표
      <ID> <제목>.md
  개념/<개념명>.md                  # 위키 노트 (한 줄짜리 용어도 노트 1개)
  개념/개념.base                    # 전체 / 분류별 / 확인필요 / 근거없음 뷰
  첨부/스크린샷/<프로젝트명>/
  첨부/다이어그램/<프로젝트명>/         # SVG 등 직접 그린 분석 다이어그램
  첨부/목업/<프로젝트명>/               # 코드 이슈의 UI 목업 HTML (승인 전 화면 확인용)
```

- 프로젝트/연구 구분은 폴더가 아니라 `type` 프로퍼티. 둘 다 `프로젝트/` 아래에 둔다.
- **이슈의 화면 구분도 폴더가 아니라 `url` 프로퍼티.** `이슈/`는 평면 폴더이고 화면별 목록은 `화면별` 뷰(`groupBy: url`)가 만든다. 폴더는 한 축으로만 나누지만 프로퍼티는 중요도·출처·상태와 함께 걸러진다.
...
- **역도 성립한다: 노트 프로퍼티에서 파생되는 목록을 마크다운 표로 유지하지 않는다.** 상태가 두 곳에 있으면 반드시 어긋난다(실제로 어긋났다 — 같은 문제가 두 절에 다른 상태로, 새 문제는 한쪽에만). 목록은 `.base` 뷰를 만들고 `![[X.base#뷰]]` 로 임베드한다.
- Bases 임베드는 **필터를 덧붙일 수 없다.** 그래서 범위마다 `.base` 파일이 하나씩 필요하다(볼트 전체 / 프로젝트). 뷰 정의가 몇 벌 늘어나는 것은 데이터가 두 벌 되는 것과 다른 문제다 — 뷰는 어긋나도 열이 달라질 뿐이고, 데이터는 거짓말을 한다.
- `.base` 이름은 볼트 안에서 유일해야 한다(위키링크·임베드가 이름으로 해석된다). 프로젝트 폴더의 것은 `<idPrefix> 이슈.base`, 인박스는 `<idPrefix> 이슈목록.md`.
...
| 노트 | frontmatter |
|---|---|
| 프로젝트 허브 | `type: 프로젝트\|연구`, `status: 예비\|진행중\|완수\|보류`, `client`, `period`, `parent`, `related[]`, `codebase`, `vcs: git\|svn`, `tags[]` |
...
| 이슈 | `type: 이슈`, `id`, `issue_type: 버그\|기능\|작업\|질문`, `execution_type: 코드\|문서\|조사\|협의\|결정`, `labels[]`, `assignees[]`, `milestone`, `status: 제안\|승인대기\|승인\|진행중\|부분완료\|완료\|보류\|취소`, `state: open\|closed`, `approval_required`, `approve`, `closed`, `commits[]`, GitHub 연결 및 승인·검증·의존성 필드 |
| 마일스톤 | `type: 마일스톤`, `id`, `status: 계획\|진행중\|완료\|보류`, `state`, `due`, GitHub 연결 필드 |

규칙: 스킬이 임의 필드를 만들지 않는다. 모르는 값은 비워둔다. 값은 한국어.

`priority` 는 유형을 가로지르는 공통 필드다(`중요.base` · 대시보드가 모은다). 다른 유형은 `최우선` · `중요` 만 쓰고 보통이면 비워 두지만, **개선 노트만 `보통` 도 적는다** — 미해결 문제를 중요도로 줄 세워야 다음에 무엇을 할지 정해지기 때문이다. 닫힌 건(`구현완료` · `반려`)은 `보통` 으로 내린다.

`url` 은 요청 단위 URL 하나(`fdrList.do`)다. 설계서상 화면 ID·화면 영역명을 프로퍼티로 두지 않는다 — 표기가 갈려 뷰가 지저분해지고, 화면 안 위치는 노트 제목과 `## 문제상황` 이 이미 말한다.
...
- 문제 ID 는 프로젝트 단위 단조 증가(`FDR-001`). 화면이 달라도 번호를 이어 써 커밋 메시지에서 전역 유일하게 한다.
...
- **파생 문서는 표를 갖지 않는다.** `개선.md` 의 상태별 절과 `<idPrefix> 문제목록.md` 의 목록은 전부 base 뷰 임베드이고, 문서가 직접 적는 것은 프로퍼티에서 나오지 않는 것뿐이다 — 목업 파일 링크, 파일 겹침 교차 검토, 구현 순서 제약, 인수인계 확인 결과. `vault-hygiene.ps1` 이 `[파생표]` 로 재발을 감시한다.
...
- 화면이 바뀌는 문제는 승인 전에 자기완결 HTML 목업(`첨부/목업/<프로젝트명>/<ID>-<안>.html`)을 만들어 브라우저로 보여주고, Playwright 캡처를 `## 설계` 의 `### 목업` 절에 임베드한다. 목업은 현재 화면·기존 시안을 기준으로 하고 없는 요소를 지어내지 않는다. MCP 부재 시 캡처만 건너뛴다.
- 설정: `config.json` 의 `improve.projects.<프로젝트명>` (`path`, `workBranch`, `portableBase`, `idPrefix`, `verify`). `workBranch` 는 남의 변경이 섞이지 않은 지점에서 분기해 두고, 승인 후 이식은 `git rebase --onto <portableBase> <분기점>` 으로 한다 — SHA 가 바뀌므로 문제 노트 `commits` 갱신 필요. PC마다 다르므로 setup 이 대화로 수집한다.
...
4. 프로젝트 허브 = MOC: 분석 문서·주요 개념·산출물 링크 목록을 최신으로 유지.
5. 프로퍼티는 템플릿 스키마만 사용.
...
- 수기 기록은 삭제하지 않고 재가공한다: 한 일 → 자동 영역 통합, 할 일 → `## 내일 할 일`, 개념·구조 설명 → 개념 노트 + `[[링크]]`, 프로젝트/개선 상세 → 해당 노트 + 링크, 그 외 메모 → `## 비고`, 애매하면 원문 그대로 보존.
...
| `project-doc` | 프로젝트 등록: 제안서(docx: pandoc, 없으면 Word COM) + 코드베이스 경로 → 프로젝트 폴더/허브/요약 |
...
- 정부프로젝트 특성상 세션 내용은 vault(로컬)에만 저장. 외부 동기화 가정 없음.
