...
- 플러그인/마켓플레이스/repo 이름: `sawhorse`. GitHub: `a7garden/sawhorse`. 스킬 호출은 `/sawhorse:<skill>`.
- 프로퍼티 키는 영어, 값은 한국어. 필드는 `docs/design.md` 스키마만 허용.
- vault 경로 표기: 스킬 본문에서 `${user_config.vault_path}`. 저널 위치: `%USERPROFILE%\.claude\sawhorse\journal\YYYY-MM-DD.jsonl`.
...
```json
{
  ...
  "version": "0.1.0",
  "author": { "name": "won" },
  "homepage": "https://github.com/a7garden/sawhorse",
  "repository": "https://github.com/a7garden/sawhorse",
  "license": "MIT",
...
`block-push.ps1`:
```powershell
# sawhorse PreToolUse hook: deny remote-mutating VCS commands (company policy).
$ErrorActionPreference = 'SilentlyContinue'
$raw = [Console]::In.ReadToEnd()
if (-not $raw) { exit 0 }
$e = $raw | ConvertFrom-Json
...
if ($deny) {
  @{
    hookSpecificOutput = @{
      ...
      permissionDecisionReason = 'sawhorse 회사 정책: 원격 저장소 변경(git push / svn commit)은 금지되어 있습니다. 로컬 작업만 진행하세요.'
    }
...
**Interfaces:**
- wiki는 규범의 단일 진실원천. 다른 스킬 본문은 "sawhorse:wiki 규범 준수"로 참조.
...
**Interfaces:**
- 저널: `%USERPROFILE%\.claude\sawhorse\journal\<오늘>.jsonl`, 라인 스키마 `{ts, session_id, cwd, transcript_path, reason}`. 같은 session_id 여러 줄 → 마지막 유효.
...
- [ ] 한국어 README: 소개, 전제조건(Windows/Claude Code/선택: Node·pandoc), 설치 2줄(`/plugin marketplace add a7garden/sawhorse` → `/plugin install sawhorse@sawhorse`), 설치 후 vault 경로 프롬프트 안내, `/sawhorse:init-vault` 첫 실행, 명령 표(6 스킬), 훅 설명(저널·push 차단), vault 구조 예시, 문제해결(MCP 실패 시 무해, 훅 로그), 라이선스.
...
- [ ] `gh repo create a7garden/sawhorse --public --source . --push`
