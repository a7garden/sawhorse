...
```
<VAULT>/
  대시보드.md      # 시작 화면 — 일지·이슈·마일스톤·사업·개념 요약
  템플릿/          # 사업, 개념, 일지, 기능분석, 회의, 이슈, 마일스톤 템플릿
  일지/            # YYYY-MM-DD.md — 업무일지 (+ 일지.base)
  사업/<사업명>/   # <사업명>.md 허브 + 분석/ + 산출물/ + 회의/ + 이슈/ + 마일스톤/
  개념/            # 위키 노트 (+ 개념.base)
...
`.base` 파일은 Obsidian Bases의 뷰 정의입니다. 개념·사업·이슈·마일스톤·일지를 표로 훑어보는 용도이며, **지식은 항상 노트에 있습니다** — `.base`를 전부 지워도 잃는 것은 보기 방식뿐입니다. 거꾸로, 노트 프로퍼티에서 나오는 목록을 문서에 마크다운 표로 베껴 두지 않습니다(상태가 두 곳에 있으면 반드시 어긋납니다). 시작 화면으로 `대시보드.md`를 열려면 커뮤니티 플러그인 **Homepage**를 설치하세요 (init-vault가 설정을 잡아줍니다).
