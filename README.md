...
`intent.md`로 원하는 결과를 정의하거나 TDD 작업을 만들고, 워크플로우별 문서와 계획을 다듬고, Herdr 에이전트를 실행한다.
...
작업대에서 **작업공간 초기화**를 누른 뒤 **프로젝트**에 저장소 경로와 검증 명령을 등록한다.
**새 작업**을 만들고 `intent.md`를 작성하면 개발 흐름을 시작할 수 있다.
...
| 백로그 | 칸반 상태 변경, 작업 생성·편집, 프로젝트와 의존 작업 연결 |
...
| 프로젝트 | 저장소 경로, 프로젝트 의존성, 검증 명령, 기본 에이전트·모델 |
...
```text
의도          설계       구현       검증               배포         학습
intent.md  →  spec.md  →  plan.md  →  verification.md  →  release.md  →  learning.md
```

개발 단계는 업무 상태(`백로그 / 준비 / 진행 / 검토 / 막힘 / 완료`)와 별개다.
...
초기화는 반복해도 기존 파일이나 공개된 워크플로우 정의를 덮지 않으며 예전 `사업/`,
