...
폴더·템플릿 생성은 앱이 확장 매니페스트로 직접 한다. 앱을 쓰면 `init-vault` 는 이미 끝난
일을 건너뛰고 **Obsidian 쪽 설정**(설치 여부·기존 설정 병합 판단이 필요한 부분)만 맡는다.
플러그인만 쓰는 경우에는 `init-vault` 가 배치까지 전부 한다 — 양쪽 다 기존 파일은 덮지 않는다.
